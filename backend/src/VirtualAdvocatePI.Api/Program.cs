...
static async Task<bool> UserOwnsWorkspaceAsync(VirtualAdvocateDbContext db, Guid userId, Guid workspaceId)
{
    return await db.ClaimWorkspaces.AnyAsync(x => x.Id == workspaceId && x.UserId == userId && x.Status != "ARCHIVED");
}

static async Task<bool> UserOwnsConditionAsync(VirtualAdvocateDbContext db, Guid userId, Guid workspaceId, Guid conditionId)
{
    return await db.ClaimWorkspaces.AnyAsync(x => x.Id == workspaceId && x.UserId == userId && x.Status != "ARCHIVED")
        && await db.ClaimConditions.AnyAsync(x => x.Id == conditionId && x.ClaimWorkspaceId == workspaceId && x.Status != "ARCHIVED");
}
